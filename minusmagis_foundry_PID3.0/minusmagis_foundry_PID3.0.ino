...
int errorCounter = 0;  //Counter to count the amount of times the thermocouple is disconected
...
void Rotary() {
  ...
  if (aState != aLastState) {
    ...
    velocity = log10(abs(1000000/(now - previousT))+1);
    previousT = now;
    //Serial.println(velocity); debugging purposes
    // If the outputB state is different to the outputA state, that means the encoder is rotating clockwise
    if (bState != aState) {
      counter = counter + 1*velocity*velocity*velocity*velocity;
    } else {
      counter = counter - abs(1*velocity*velocity*velocity*velocity);
    }
  }
